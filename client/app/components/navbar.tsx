...
export default function Navbar(){
    ...
    const debounce_first = (func: any, timeout: number): (...args: any[]) => void => {
        let timer: any;
        return (...args: any[]) => {
            if (!timer) {
                func(...args);
    ...
    return (
        <nav className="flex flex-row items-center justify-between bg-white dark:bg-[var(--dark-background-col)] p-4">
            ...
            {showClose && (
                <div className="search-container rounded-s relative w-6/12 flex flex-col border-black-100 dark:border-white border-3 p-1 focus-within:border-[var(--accent-col)]">
                    <label htmlFor="art-search" className="text-sm">Search</label>
                    <input type="text" id="art-search" className="bg-transparent focus:outline-none" value={query} onKeyUp={handleAutocomplete} onChange={(e) => setQuery(e.target.value)} />
                   ...
                   {autocomplete.length > 0 && (
                    ...
                    <ul className="absolute dark:bg-[var(--dark-background-col)] w-full top-full left-0 h-auto border-3 p-1 border-t-0 border-[var(--accent-col)]">
